...
static BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	// always use own keymap
    char keymapfilename[FILENAMELEN];
    sprintf(keymapfilename, "%s\\keymapx.txt", free42dirname);
    read_key_map(keymapfilename);

	// always use own printfile
	sprintf(printfilename, "%s\\printx.bin", free42dirname);
	...
	// if no free42x state file, try to import free42 state file
    int init_mode;
    int4 version;
    sprintf(statefilename, "%s\\statex.bin", free42dirname);
	...
	/* initialization of port for hpil emulation */
	shell_init_port();
    ...
    switch (message)
    {
        case WM_INITDIALOG: {
            ...
            if (core_settings.enable_ext_hpil) {
				...
				if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Hardware\\DeviceMap\\SerialComm", 0, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &hkey) == ERROR_SUCCESS) {
					do {
						...
						if ((err ==ERROR_SUCCESS) && (hkeyValueLen > 4)) {
							...
							if (!strcmp(hkeyValue,state.comPort)) {
				...
				if (!strcmp("TCP/IP",state.comPort)) {
				...
				SendMessage(ctl, IPM_SETADDRESS, 0, (LPARAM)state.outIP);
				sprintf(hkeyValue, "%u", state.outTcpPort);
                ctl = GetDlgItem(hDlg, IDC_HPIL_TCP_PORT_OUT);
				SendMessage(ctl, WM_SETTEXT, 0, (LPARAM)hkeyValue);
				sprintf(hkeyValue, "%u", state.inTcpPort);
        ...
        case WM_COMMAND: {
            ...
            switch (cmd) {
                case IDOK: {
					...
					SendMessage(ctl, CB_GETLBTEXT, dwindex, (LPARAM)state.comPort);
					state.highSpeed = false;
					state.medSpeed = false;
                    ctl = GetDlgItem(hDlg, IDC_HPIL_HIGH_SPEED);
                    ...
                    ctl = GetDlgItem(hDlg, IDC_HPIL_IP_ADD);
					SendMessage(ctl, IPM_GETADDRESS, 0, (LPARAM)&state.outIP);
                    ctl = GetDlgItem(hDlg, IDC_HPIL_TCP_PORT_OUT);
					((WORD*)hkeyValue)[0] = 64;
...
static void init_shell_state(int4 version) {
    switch (version) {
        ...
        case 7:
			...
			state.medSpeed = true;
			state.pilBox = false;
			state.outIP = 0x7f000001;
			state.inTcpPort = 60001; 
			state.outTcpPort = 60000; 
            // fall through
		case 8:
...
static int shell_init_IP() {
	...
	// open server socket
	sprintf(portStrBuf, "%u", state.inTcpPort);
	...
	hints.ai_socktype = SOCK_STREAM;
	hints.ai_protocol = IPPROTO_TCP;
	sprintf(ipStrBuf, "%u.%u.%u.%u", state.outIP>>24, state.outIP>>16&0xff, state.outIP>>8&0xff, state.outIP&0xff);
	sprintf(portStrBuf, "%u", state.outTcpPort);
...
static void shell_init_port() {
	...
	if (!strcmp(state.comPort," Disabled")) {
	}
	else if (!strcmp(state.comPort,"TCP/IP")) {
		shell_close_IP();
		if (!shell_init_IP()) {
			modeEnabled = true;
			modeIP = true;
